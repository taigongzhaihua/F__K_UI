...
**Windows**:
- 使用动态链接库（DLL）：`glfw3dll.lib` + `glfw3.dll`
- 定义 `GLFW_DLL` 宏
...
|------|---------|-------|-------|
| GLFW 库 | DLL (glfw3dll.lib) | 静态 (libglfw3.a) | 静态 (libglfw3.a) |
| GLFW_DLL 宏 | ✅ 定义 | ❌ 不定义 | ❌ 不定义 |
| X11 依赖 | ❌ 不需要 | ⚠️ 可选 | ❌ 不需要 |
| OpenGL | ✅ 内置 | ⚠️ 可选 | ✅ 内置 |
...
确保 `third_party/lib/` 目录包含正确的库文件：
- **Windows**: `glfw3dll.lib`, `glfw3.dll`
- **Linux**: `libglfw3.a`
