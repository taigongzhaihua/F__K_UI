...
**⚠️ 重要调整**：经过对 TextBlock 实现的分析，发现 Render 模块核心功能未实现，因此 **Phase 5 首先重新设计并实现 Render 模块**（Phase 5.0），这是所有后续功能的基础。
...
### 主要目标
1. **🔥 重新设计并实现 Render 模块** - RenderContext、文本渲染、Shape 渲染（基础设施，P0）
2. **补充文本输入控件** - TextBox, RichTextBox, PasswordBox（关键缺失）
3. **完善交互系统** - 拖放、命令、快捷键
4. **建立测试体系** - 单元测试、集成测试、性能测试
5. **完善文档系统** - 教程、API 文档、示例应用
6. **确保生产就绪** - Bug 修复、性能优化、跨平台支持
...
### 5.0 Render 模块重新设计与实现 (2.5周) 🔥 **最高优先级 - 基础设施**

**状态**: ⏳ 进行中  
**文档**: 详见 `Phase5-RenderModuleRedesign.md`

#### 核心问题
- ❌ TextBlock 无法渲染（CollectDrawCommands 未实现）
- ❌ RenderContext 类定义缺失
- ❌ 文本度量不准确（使用简单估算）
- ⚠️ 双重命令系统架构不清晰
...
```
UI 层 (UIElement::CollectDrawCommands)
    ↓
RenderContext 层 (高层绘制 API + 状态管理) ← 新增
    ↓
RenderCommand 层 (统一命令格式)
    ↓
后端层 (GlRenderer, VulkanRenderer...)
```
...
**总计**: ~2,450 行代码，13 天（约 2.5 周）

#### 成功标准
- ✅ TextBlock 可正确渲染文本
- ✅ 1000 个 TextBlock 渲染: 60 FPS
- ✅ 文本度量准确（误差 < 5%）
- ✅ 所有 Shape 可正确渲染
- ✅ 清晰的架构分层
...
|------|---------|---------|---------|------|
| **5.0 Render 重新设计** | **2,450** | **-** | **550** | **3,000** |
...
| **总计** | **18,250** | **10,900** | **11,550** | **40,700** |

**注**: Phase 5.0 测试已包含在其代码量中（~600 行测试程序）

### 时间估计（更新版）

| 阶段 | 预估时间 |
|------|---------|
| **5.0 Render 重新设计** | **2.5 周** ← 新增 |
| 5.1 文本输入 | 2-3 周 |
| 5.2 高级控件 | 1-2 周 |
| 5.3 布局容器 | 1 周 |
| 5.4 交互系统 | 1-2 周 |
...
**建议时间**: **16 周**（4 个月）← 比原计划增加 2 周

### 人力需求

- **建议**: 2-3 开发者
- **最少**: 1 开发者（时间会延长到 20-22 周）
- **理想**: 3 开发者
  - 开发者 1: Render 模块 + 核心控件（5.0-5.2）
  - 开发者 2: 测试和质量保证（5.6）
...
### 优先级原则
1. **P0（关键）**: 🔥 **Render 模块重新设计**、文本输入控件、测试框架
2. **P1（高）**: 高级控件、文档
3. **P2（中）**: 工具、跨平台
4. **P3（低）**: 优化打磨

### 开发顺序建议（更新版）
1. **Week 1-2.5**: 🔥 **5.0 Render 模块重新设计**（基础设施，最优先）
2. **Week 3-5**: 5.1 文本输入控件（TextBox 优先）
3. **Week 6-7**: 5.2 高级控件（TreeView, TabControl 优先）
4. **Week 8-9**: 5.4 交互系统（拖放、命令）
5. **Week 10-12**: 5.6 测试体系（最关键！）
6. **Week 13-14**: 5.7 文档和示例
7. **Week 15**: 5.8-5.9 工具和跨平台
8. **Week 16**: 5.10 最终打磨和发布

**总时长**: 16 周（约 4 个月）← 比原计划增加 2 周（因为增加了 Render 模块重新设计）
