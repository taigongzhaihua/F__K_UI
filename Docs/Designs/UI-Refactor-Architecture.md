...
- 为后�?ControlTemplate、资源系统、主题体系等扩展留出清晰的挂接点�?
...
- 提供 `SetValue`/`GetValue`、属性变更通知、绑定目�?源的注册点�?
...
- 提供统一的子节点集合管理器（例如内部 `VisualCollection` 或外�?`VisualTreeManager`），屏蔽不同派生类对视觉树操作的差异�?
- 定义渲染数据生成流程：`BuildRenderTree(RenderContext&)` �?`CollectDrawCommands(Renderer&)`�?
- 与渲染器 `Renderer` 解耦：Visual 只输�?`DrawCommand`，Renderer 决定如何执行�?

### 2.3 UIElement

- 引入输入与布局的核心能力：命中测试、输入事件冒�?隧道、`Measure/Arrange` 模型�?
...
- 管理尺寸相关属性：`Width`、`Height`、`Min/Max`，并�?`Measure/Arrange` 协调�?
...
- 支持主题样式查找顺序：显式样�?�?资源 �?默认主题�?

### 2.6 ContentControl

- �?`Control` 继承，增�?`Content`、`ContentTemplate`�?
- 接受任意对象�?UIElement 作为内容，若为数据对象则通过模板生成为视觉树�?
- 常用控件（`Button`、`Window` 等）基于此实现�?

### 2.7 ItemsControl

- 控件的集合化版本：管�?`ItemsSource`、`ItemsPanel`、`ItemTemplate`�?
- 定义 `ItemContainerGenerator`，负责数据项 �?UI 容器的生命周期�?
- 事件和选择状态由 `Selector` 等派生类扩展�?

### 2.8 Panel

- 直接继承�?`FrameworkElement`（保持与 WPF/WinUI 一致），避免额外的内容包装开销�?
- 提供子元素集合管理，并覆�?`MeasureOverride`、`ArrangeOverride` 实现不同布局策略�?
- �?`ItemsControl` 配合，通过 `ItemsPanel` 指定布局容器类型�?
- 不引�?`ContentControl` 层级，布局容器专注于多子元素排列，逻辑树（父子关系）与视觉树保持一一对应�?

### 2.9 常用控件继承关系示例

- **轻量元素（直接继�?`FrameworkElement`�?*
  - `TextBlock �?FrameworkElement`
  - `Image �?FrameworkElement`
  - `Border �?Decorator �?FrameworkElement`
  - `Rectangle`/`Ellipse`/`Path` �?�?`Shape �?FrameworkElement`
- **布局容器**
...
| 阶段 | 主要类型 | 职责 | 可选辅助组�?|
| --- | --- | --- | --- |
| 控件逻辑 | `Control` / `ContentControl` / `ItemsControl` | 管理属性、状态、命令等交互逻辑；在 `OnApplyTemplate` 中触发模板应�?| `VisualStateManager`（状态机），`InteractionTracker` |
| 模板实例�?| `ControlTemplate` + `TemplateBuilder` | 依据样式生成实际的元素树，解析命名部件并绑定到控�?| `TemplatePartResolver`（定位模板部件），`TemplateCache` |
| 视觉树挂�?| `Visual` + `VisualCollection`/`VisualTreeManager` | 维护父子关系，确保模板生成的元素正确插入视觉树；处理变换与剪�?| `VisualInvalidationScope`（批量无效化�?|
| 绘制命令生成 | `Visual::CollectDrawCommands` | 根据当前状态生�?`DrawCommand` 序列 | `RenderTreeBuilder`（将复杂控件拆解为命令序列），`GeometryCache` |
| 顶点/纹理准备 | `Renderer` + `RenderTreeCompiler` | 将命令映射到后端资源，构建顶�?索引缓冲或文本布局 | `MaterialPool`、`DynamicMeshBuffer` |
...
- 依赖属性变更通过 `PropertyChanged` 事件向上冒泡，必要时触发布局/渲染无效化：
  - 外观属性变�?�?`InvalidateVisual()`�?
  - 布局属性变�?�?`InvalidateMeasure()` �?`InvalidateArrange()`�?
...
- 输入管理�?
  - `InputManager` 收集平台事件，命中测试确定目�?`UIElement`�?
...
- `UIElement` 提供公共实现，`Panel` �?`FrameworkElement` 通过虚函数自定义�?
- 支持布局无效化：尺寸相关依赖属性变更时重启流程�?

## 7. 样式与模�?

- 样式系统依托依赖属性：
  - `Style` 绑定属�?setter、触发器�?
...
- 新增控件通常派生�?`Control` �?`ContentControl`，覆�?`OnApplyTemplate` 获取模板部件�?
- 自定义布局容器继承 `Panel`，实�?`MeasureOverride` �?`ArrangeOverride`�?
...
- **视觉�?*：`Visual`、`VisualCollection`（或同等管理器）、`HitTestResult`�?
- **输入与布局�?*：`UIElement`、`InputManager`、`FocusManager`、`MeasureContext`/`ArrangeContext`�?
- **框架�?*：`FrameworkElement`、`ResourceDictionary`、`Style`、`StyleSelector`、`FrameworkTemplate`、`DataTemplate`�?
- **控件�?*：`Control`、`ContentControl`、`ItemsControl`、`ItemContainerGenerator`、`ControlTemplate`、`TemplateBuilder`�?
...
- `std::any GetValue(DependencyProperty* dp) const`：读取当前有效值（本地�?�?样式 �?默认值）�?
...
**职责**：封�?Visual 的子节点集合，提供类型安全的操作接口�?
...
- `void SetWidth(float value)` / `float GetWidth() const`：设�?获取宽度�?
- `Derived* Width(float value)`：链式调用设置宽度�?
- `float Width() const`：无参重载，获取宽度�?
- `void SetHeight(float value)` / `float GetHeight() const`：设�?获取高度�?
...
### 11.18 ItemContainerGenerator

**职责**：管理数据项�?UI 容器的映射和生命周期�?

**继承**：无（容器生成器辅助类）

- `UIElement* Generate(const std::any& item, int index)`：为数据项生�?UI 容器�?
...
- `Derived* AddChild(UIElement* child)`：添加子元素并返�?this（链式调用）�?
...
- `static void SetRowSpan/ColumnSpan(UIElement*, int span)`：设置跨�?列数�?
...
为支持流畅的链式调用风格（如 `button->Content("Click")->Padding(5, 10)`），框架中所有可设置的属性都提供三种形式�?API�?

1. **传统 Getter/Setter**：`SetXxx(value)` / `GetXxx()`，遵循明确命名约定�?
2. **链式 Setter**：`Xxx(value)` 返回 `this` 指针，支持方法链�?
3. **无参 Getter**：`Xxx()` 无参重载返回属性值，与链�?setter 同名�?
...
auto content = button->Content();  // 无参重载获取�?
```

### 12.2 指针类型选择

链式 API 返回**裸指�?*而非智能指针，原因如下：

- **生命周期由外部管�?*：UI 元素的生命周期由父容器或资源系统控制，链式调用不应转移所有权�?
- **避免智能指针开销**：返�?`std::shared_ptr<T>` 会强制整个调用链使用智能指针，增加引用计数开销且与树结构管理冲突�?
...
**实现注意事项**�?
- 每个可实例化的类需�?封闭"CRTP 链，传入自身类型（如 `class Button : public Control<Button>`）�?
- 纯抽象基类（�?`Control` 本身作为基类）保持模板形式，由具体类完成实例化�?
- 便捷重载（如 `Padding(float uniform)`）在派生类中补充，以保持接口简洁�?

### 12.4 实现规则

- 链式 setter 返回 `Derived*`（通过 CRTP），便于类型安全的链式调用�?
- 便捷重载（如 `Padding(float uniform)` �?`Padding(float left, float top, float right, float bottom)`）提升易用性�?
...
3. **属性访问封�?*：getter/setter 通过 `GetValue<T>()`/`SetValue()` 访问
   ```cpp
...
#### FrameworkElement�? 个属性）
| 属性名 | 类型 | 默认�?| 说明 |
|--------|------|--------|------|
| `Width` | `float` | `-1.0f` | 宽度�?1 表示自动�?|
| `Height` | `float` | `-1.0f` | 高度�?1 表示自动�?|
| `MinWidth` | `float` | `0.0f` | 最小宽�?|
| `MaxWidth` | `float` | `∞` | 最大宽�?|
| `MinHeight` | `float` | `0.0f` | 最小高�?|
| `MaxHeight` | `float` | `∞` | 最大高�?|
| `DataContext` | `std::any` | `std::any()` | 数据上下�?|

**待添加属�?*：`Margin`, `HorizontalAlignment`, `VerticalAlignment`
...
| `Padding` | `Thickness` | `Thickness(0)` | 内边�?|

**待添加属�?*：`FontFamily`, `FontSize`, `FontWeight`
...
3. **模板类特殊处�?*�?
   ...
   - 使用静态局部变量确保单例注�?
   - 通过 `this->template GetValue<T>()` 在模板类中调�?

4. **链式 API 兼容**：所有链式调用接口保持不变，内部委托到依赖属性系统�?

5. **类型安全**：使�?`typeid()` 确保类型匹配，运行时检查属性类型�?
...
| IsEnabled | `bool` | `true` | 是否启用 | �?|
| Opacity | `float` | `1.0f` | 不透明�?| �?|
| Clip | `Geometry*` | `nullptr` | 裁剪区域 | 📋 待实�?|
| RenderTransform | `Transform*` | `nullptr` | 渲染变换 | 📋 待实�?|

#### FrameworkElement (已实�?7/10)
| 属性名 | C++ 类型 | 默认�?| 说明 | 状�?|
|--------|----------|--------|------|------|
| Width | `float` | `-1.0f` | 宽度 | �?|
...
| MaxHeight | `float` | `∞` | 最大高�?| �?|
| DataContext | `std::any` | `std::any()` | 数据上下�?| �?|
| Margin | `Thickness` | `Thickness(0)` | 外边�?| �?|
...
| ItemTemplate | `DataTemplate*` | `nullptr` | 项模�?| �?|
| ItemsPanel | `UIElement*` | `nullptr` | 项容器面�?| �?|
...
| Spacing | `float` | `0.0f` | 子元素间�?| �?|

#### Grid (未实�?0/0)
**注意**：Grid �?Row/Column 是附加属性，使用全局 map 实现，不计入依赖属性系�?
...
| Foreground | `Brush*` | `nullptr` | 前景�?| �?|
| TextWrapping | `TextWrapping` | `NoWrap` | 文本换行 | �?|

#### Border (已实�?6/6)
| 属性名 | C++ 类型 | 默认�?| 说明 | 状�?|
|--------|----------|--------|------|------|
| Child | `UIElement*` | `nullptr` | 子元�?| �?|
| BorderBrush | `Brush*` | `nullptr` | 边框画刷 | �?|
| BorderThickness | `Thickness` | `Thickness(0)` | 边框厚度 | �?|
...
#### Button (未实�?0/0)
**注意**：Button �?ContentControl 继承，无额外依赖属�?

#### Window (已实�?6/6)
| 属性名 | C++ 类型 | 默认�?| 说明 | 状�?|
|--------|----------|--------|------|------|
| Title | `std::string` | `""` | 窗口标题 | �?|
| WindowState | `WindowState` | `Normal` | 窗口状�?| �?|
| Left | `float` | `0.0f` | 窗口左边�?| �?|
| Top | `float` | `0.0f` | 窗口顶边�?| �?|
...
#### 第一批（核心布局和外观）�?已完�?
1. �?UIElement - 基础可见性和交互�? 个属性）
2. �?FrameworkElement - 尺寸约束和数据上下文�? 个属性）
3. �?Control - 外观属性（5 个属性）
4. �?ContentControl - 内容展示�? 个属性）

#### 第二批（文本和容器）�?已完�?
5. �?TextBlock - 文本显示�? 个属性）
6. �?Border - 装饰容器�? 个属性）
7. �?Panel - 布局基类�? 个属性）
8. �?StackPanel - 线性布局�? 个属性）

#### 第三批（集合控件）✅ 已完�?
9. �?ItemsControl - 集合控件�? 个属性）

#### 第四批（扩展属性）�?已完�?
10. �?FrameworkElement 扩展 - Margin、HorizontalAlignment、VerticalAlignment�? 个属性）
11. �?Control 扩展 - FontFamily、FontSize、FontWeight�? 个属性）

#### 第五批（窗口类）�?已完�?
12. �?Window - 顶层窗口容器�? 个属性）

#### 第六批（可选扩展）📋 未实�?
13. ⏸️ UIElement 扩展 - Clip、RenderTransform�? 个属性，需 Geometry �?Transform 类型支持�?
14. ⏸️ Button 扩展 - ClickMode、IsDefault、IsCancel�? 个属性，可选功能）
15. ⏸️ 其他控件特定属性（按需扩展�?
...
1. **Window** (ContentControl �?Window)
   - `include/fk/ui/Window.h` - 窗口类声�?
   - `src/ui/Window.cpp` - 窗口类实�?

#### 新增依赖属性实现文�?
1. `src/ui/Panel.cpp` - Panel 模板类依赖属性注�?
2. `src/ui/ItemsControl.cpp` - ItemsControl 模板类依赖属性注�?
3. `src/ui/Window.cpp` - Window 类依赖属性注册及窗口操作

### 13.9 后续优化方向

1. **属性变更回�?*：为关键属性添�?`PropertyChangedCallback`，实现自定义逻辑�?
2. **属性值继�?*：实�?`DataContext` 等属性的自动继承机制�?
3. **附加属性支�?*：实�?`Grid.Row`、`Grid.Column` 等附加属性（当前使用全局 map）�?
4. **属性验�?*：为数值属性添加范围验证（�?Opacity 0.0-1.0）�?
5. **性能优化**：对频繁访问的属性添加缓存机制�?
6. **类型系统完善**：实�?Geometry、Transform 基类以支�?Clip �?RenderTransform 属性�?
...
5. 📋 编写核心单元测试：依赖属性、绑定冒泡、布局无效化、事件路�?
6. 📋 实现基础 Renderer �?Canvas，验�?DrawCommand 流程
7. 📋 迁移现有控件到新层级，确保示例程序可运行
8. 📋 梳理资源、主题系统与新架构的对接方案
9. 📋 实现属性值继承和附加属性机�?
10. 📋 实现 Geometry �?Transform 基类，支�?Clip �?RenderTransform 属�?
