...
#include "../base/FrameworkElement.cpp"
...
Size Shape<Derived>::MeasureOverride(const Size& availableSize) {
    // Shape 使用显式设置�?Width/Height，如果没有设置则使用几何边界
    float width = this->GetWidth();
    float height = this->GetHeight();
    
    // 如果没有显式设置 Width/Height (默认值为 -1.0)，使用几何边�?
    ...
    // GetDefiningGeometry 已经包含了线宽的计算，直接返�?
...
void Rectangle::OnRender(render::RenderContext& context) {
    ...
    // �?Brush 获取颜色（转换为 RenderContext 格式�?
    ...
    // 使用新的 DrawRectangle API，支�?radiusX �?radiusY
...
void Polygon::OnRender(render::RenderContext& context) {
    if (points_.size() < 3) return;  // 需要至�?个点才能构成多边�?
...
void Path::OnRender(render::RenderContext& context) {
    ...
    // �?ui::Path::PathSegment 转换�?render::PathSegment
